...
class Box(object):
    ...
    def double(self):
        ...
        if len(self.contents) < extra_vol:
            self.contents = self.contents + self.contents
            return ''
        else:
            for k in range(extra_vol):
                string_to_add = contents[k]
                self.contents = self.contents + string_to_add

            leftover_vol = len(contents) - extra_vol
            for k in range(leftover_vol):
                return_string = return_string + contents[
                    extra_vol + k]
            return return_string
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) <= new_volume:
            ...
            for k in range(len(self.contents) - new_volume):
                return_string = return_string + self.contents[new_volume
                                                             +k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        total_discarded_letters = len(double) + len(shrink)
        return total_discarded_letters
    ...
    def steal(self, other_box):
        ...
        self.contents = self.orig_contents
        new_other_contents = self.append_string(other_box.contents)
        other_box.contents = new_other_contents
